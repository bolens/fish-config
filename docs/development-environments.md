...
The shell supplies Fish, Bash, GNU utilities, Git, Python, Ruff, ShellCheck, Markdown lint, actionlint, and Zizmor. `repo-check` runs the existing pre-push hook, checking each Fish file separately without executing it, then validates documentation and the container adapter. It does not load this checkout as your active configuration or update Fisher plugins. Syntax checks do not establish startup speed or optional integration behavior.
...
Apple execution is not verified by Linux tests. Native macOS CI checks repository tooling and Fish syntax; the configuration's Arch/Omarchy runtime integrations remain Linux-specific. Follow [the delivery playbook](../RELEASING.md) for source delivery and separate live installation.
